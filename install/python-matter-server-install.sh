...
# create_self_signed_cert                  # Creates cert in /etc/ssl/python-matter-server/
...
if ! curl -Lo /usr/local/bin/chip-ota-provider-app "https://github.com/home-assistant-libs/matter-linux-ota-provider/releases/download/2025.9.0/chip-ota-provider-app-x86-64"; then
    msg_error "Failed to download chip-ota-provider-app"
    exit 1
fi
...
fetch_and_deploy_gh_release "python-matter-server" "matter-js/python-matter-server" "tarball" "latest" "/opt/python-matter-server"
...
msg_info "Configuring python-matter-server"
mkdir /data
msg_ok "Configured python-matter-server"
...
cat <<EOF >/etc/systemd/system/python-matter-server.service
[Unit]
Description=python-matter-server Service
...
EOF
systemctl enable -q --now python-matter-server
