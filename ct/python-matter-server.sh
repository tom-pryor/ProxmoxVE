#!/usr/bin/env bash
source <(curl -fsSL https://raw.githubusercontent.com/tom-pryor/ProxmoxVE/refs/heads/feature/python-matter-server/misc/build.func)
...
function update_script() {
  ...
  if [[ ! -d /opt/python-matter-server ]]; then
    msg_error "No ${APP} Installation Found!"
    exit
  fi

  # check_for_gh_release returns 0 (true) if update available, 1 (false) if not
  if check_for_gh_release "python-matter-server" "matter-js/python-matter-server"; then
    msg_info "Stopping Services"
    systemctl stop python-matter-server
    msg_ok "Stopped Services"

    # CLEAN_INSTALL=1 removes old directory before extracting new version
    CLEAN_INSTALL=1 fetch_and_deploy_gh_release "python-matter-server" "matter-js/python-matter-server" "tarball" "latest" "/opt/python-matter-server"
    ...
    systemctl start python-matter-server
